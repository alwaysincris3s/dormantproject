...
update_gmail_credentials_ui_combined() {
    # Extract current email and password from your TARGET_SCRIPT (assuming sendemail usage)
    current_email=$(grep -Po '(?<=-xu )[^ ]+' "$TARGET_SCRIPT" | head -1)
    current_password=$(grep -Po '(?<=-xp ")[^"]+' "$TARGET_SCRIPT" | head -1)

    dialog --form "Update Gmail Credentials" 12 60 2 \
        "Gmail Email:" 1 1 "$current_email" 1 15 40 0 \
        "App Password:" 2 1 "" 2 15 40 1 2> "$TMPFILE"
    ...
    # Update sendemail line in your TARGET_SCRIPT with new email and password
