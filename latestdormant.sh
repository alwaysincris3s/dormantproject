...
# -------- Update Gmail Credentials UI --------
update_gmail_credentials_ui() {
    # Extract current email and password line from script
    current_email=$(grep -Po '(?<=-xu )[^ ]+' "$TARGET_SCRIPT" | head -1)
    current_password=$(grep -Po '(?<=-xp ")[^"]+' "$TARGET_SCRIPT" | head -1)

    masked_password=$(mask_password "$current_password")

    # Show current creds and ask if want to update email or password
    CHOICE=$(dialog --menu "Current Gmail Credentials:\nEmail: $current_email\nPassword: $masked_password\n\nChoose what to update:" 15 60 3 \
        1 "Update Email" \
    ...
    case $CHOICE in
        1)
            dialog --inputbox "Enter new Gmail address:" 8 60 "$current_email" 2> "$TMPFILE"
            new_email=$(<"$TMPFILE")
            if [[ -z "$new_email" ]]; then
                dialog --msgbox "Email cannot be empty." 6 40
                return
            fi
            new_password="$current_password"
            ;;
        2)
            dialog --insecure --passwordbox "Enter new App Password:" 8 40 2> "$TMPFILE"
            new_password=$(<"$TMPFILE")
            if [[ -z "$new_password" ]]; then
                dialog --msgbox "App password cannot be empty." 6 40
                return
            fi
    ...
    dialog --yesno "Please confirm changes:\n\nEmail: $new_email\nApp Password: $(mask_password "$new_password")" 10 50
    if [ $? -ne 0 ]; then
        dialog --msgbox "Update cancelled." 6 40
        return
    fi

    # Update the script using your sed command
    sudo sed -i -r "s|^\s*-o tls=yes -xu [^ ]+ -xp .*$|          -o tls=yes -xu $new_email -xp \"$new_password\"|" "$TARGET_SCRIPT"

    # Show updated line from script for confirmation
    updated_line=$(grep -P '^\s*-o tls=yes -xu' "$TARGET_SCRIPT")
    dialog --msgbox "Gmail credentials updated successfully.\n\nUpdated sendemail line:\n$updated_line" 10 70
}
...
# -------- Add options in dormant_patch_menu --------
dormant_patch_menu() {
    while true; do
        CHOICE=$(dialog --clear --backtitle "Dormant.sh Patching" \
            --title "Dormant.sh Patching Menu" \
            --menu "Select an option:" 15 60 4 \
            1 "Update Server URL Manually" \
            2 "Update Gmail Credentials" \
            3 "Generate New ngrok URL and Update" \
            4 "Back to Main Menu" \
            3>&1 1>&2 2>&3)

        case $CHOICE in
            1) update_server_url_ui ;;
            2) update_gmail_credentials_ui ;;
            3) generate_new_ngrok_url ;;
            4) break ;;
            *) break ;;
        esac
    done
